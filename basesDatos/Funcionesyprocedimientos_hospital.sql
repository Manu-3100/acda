CREATE FUNCTION `estatistica_medico`(medico int) RETURNS int(11)
begin
  declare n int;
...
CREATE FUNCTION `existeHabitacion`(hab_numero INT) RETURNS tinyint(1)
BEGIN
...
CREATE FUNCTION `existePaciente`(doe_numhistoria INT) RETURNS tinyint(1)
BEGIN
	Declare contador Int;
...
CREATE FUNCTION `hayCamaLibre`(hab_numero INT) RETURNS tinyint(1)
BEGIN
	DECLARE contador INT;
